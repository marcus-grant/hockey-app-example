...
# The best way to unauthenticate users locally
In the end the biggest problems with de-authentication stems from the fact that when web authentication is used to login to the hockey servers, a `SFSafariViewController` is used and directed to the login page of the Hockey website. By itself that isn't really a problem if being able to easily change users is desired. However, most accounts used by the Method team here appear to be based on Google's Single Sign On (SSO). This means that some kind of cookie or other cached data is used by the Google SSO to remember who is logged in so that single sign on truly means only one sign on is required. This means that unless the `HockeySDK`s `AuthenticationViewController` and the associated endpoints are modified to specifically handle sign offs using Google SSO, Hockey's service can't sign the user out without considerable indput from the user, using the Safari App seperately.  SafariViewControllers as opposed to a completely developer defined WebViewController which lives in the apps own sandbox shares the same cache as any other app that uses the `SafariViewController`, and because Apple has to be able to assure some level of security of this shared view controller, developers have no *permissable* access to this view controller. There seems to be some way to clear all cached data in the shared local data of Safari, which isn't ideal either. Instead, a developer defined modal instance of the `SafariViewController` can be created to take the user to Google's signout page for SSO, to invalidate Google's authentication. This along with deleting the Apple Keychain entries that the `HockeySDK` creates whenever a user is authenticated to remember that the app has been authenticated and validated, makes it possible to invalidate the local user on the iOS device, and log back in as a different user.
